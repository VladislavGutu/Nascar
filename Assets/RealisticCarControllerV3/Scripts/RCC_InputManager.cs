...
public class RCC_InputManager : MonoBehaviour{
	...
	public static RCC_Inputs GetInputs(){
		...
		case RCC_Settings.ControllerType.Nintendo:
			...
			inputs.throttleInput = Mathf.Clamp01(ReInput.players.GetPlayer(0).GetAxis(RCC_Settings.Instance.verticalInput_N));
			inputs.brakeInput = Mathf.Abs(Mathf.Clamp(ReInput.players.GetPlayer(0).GetAxis(RCC_Settings.Instance.verticalInput_N), -1f, 0f));
			inputs.steerInput = Mathf.Clamp(ReInput.players.GetPlayer(0).GetAxis(RCC_Settings.Instance.horizontalInput_N), -1f, 1f);
			// inputs.handbrakeInput = ReInput.players.GetPlayer(0).GetButton(RCC_Settings.Instance.handbrakeKB_N) ? 1f : 0f;
			// inputs.boostInput = ReInput.players.GetPlayer(0).GetButton(RCC_Settings.Instance.boostKB_N) ? 1f : 0f;
