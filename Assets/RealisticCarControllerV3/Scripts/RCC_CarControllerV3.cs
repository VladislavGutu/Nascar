...
public class RCC_CarControllerV3 : RCC_Core {
	...
	private void Inputs() {
			...
			if (canControl)
			{
				...
				if (!externalController)
				{
					...
					{
						if (!changingGear && !cutGas)
							throttleInput = (direction == 1
								? Mathf.Clamp01(inputs.throttleInput)
								: Mathf.Clamp01(inputs.brakeInput));
						else
							throttleInput = 0f;
					}
					...
					if (useSteeringSensitivity)
						steerInput = Mathf.Lerp(steerInput, inputs.steerInput,
							Time.deltaTime * steeringSensitivityFactor);
					else
	...
	private void GetExternalInputs(){
		...
		case RCC_Settings.ControllerType.PS4:
			...
			case RCC_Settings.ControllerType.Nintendo:
				if (!automaticGear)
				{

					// if (ReInput.players.GetPlayer(0).GetButtonDown(RCCSettings.shiftGearUp_N))
					// {
					//
					// 	//**RCC_UIDashboardButton -> case ButtonType.GearUp:
					// 	RCC_MobileButtons.Instance.gearDirection = 0;
					// 	RCC_MobileButtons.Instance.gearButtonUp.GetComponent<RCC_UIDashboardButton>().Check();
					//
					// 	//GearShiftUp();
					// }
					//
					// if (ReInput.players.GetPlayer(0).GetButtonDown(RCCSettings.shiftGearDown_N))
					// {
					// 	RCC_MobileButtons.Instance.gearDirection = 2;
					// 	RCC_MobileButtons.Instance.gearButtonDown.GetComponent<RCC_UIDashboardButton>().Check();
					//
					// 	//GearShiftDown();
					// }
