...
public class AutoScrollRect : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    void ScrollToSelected(bool quickScroll)
    {
        ...
        if (selectedIndex > -1)
        {
            if (quickScroll)
            {
                m_ScrollRect.normalizedPosition = new Vector2(1 - (selectedIndex / ((float)m_Selectables.Count - 1)), 0);
                m_NextScrollPosition = m_ScrollRect.normalizedPosition;
            }
            else
            {
                m_NextScrollPosition = new Vector2(1 - (selectedIndex / ((float)m_Selectables.Count - 1)), 0);
            }
        }
    }
